...
For full functionality (including OIDC/OAuth callbacks), it is recommended to run the Accounts app with SSL locally using the custom domain `conta.urbis.sampa.br`.

### Prerequisites

1.  **SSL Certificates**: You must have `conta.urbis.sampa.br.pem` and `conta.urbis.sampa.br-key.pem` in the root of `apps/accounts`.
2.  **Hosts File**: Add the following entry to your `/etc/hosts` (macOS/Linux) or `C:\Windows\System32\drivers\etc\hosts` (Windows):
    ```
    127.0.0.1 conta.urbis.sampa.br
    ```
...
*   Start a secure proxy server on `https://conta.urbis.sampa.br` (port 443).
