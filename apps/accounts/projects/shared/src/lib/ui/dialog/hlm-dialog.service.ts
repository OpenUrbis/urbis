import { Injectable, Component, Type, inject, signal, computed, Injector, InjectionToken } from '@angular/core';
...
export class HlmDialogService {
    ...
    open<T, R = any>(component: Type<T>, config?: DialogConfig): DialogRef<R> {
        ...
        const dialogRef: DialogRef<R> = {
            close: (result?: R) => {
                this._dialogs.update((d) => d.filter((dialog) => dialog.id !== dialogId));
                resolveClose(result);
            },
...
@Component({
    ...
    template: `
    <div *ngFor="let dialog of dialogs()" class="fixed inset-0 z-[9999] flex items-center justify-center">
      <!-- Backdrop -->
      <div class="absolute inset-0 bg-background/80 backdrop-blur-sm" (click)="closeDialog(dialog.id)"></div>
