...
@Component({
  ...
  template: `
    <header
      class="sticky top-0 z-[1000] h-16 w-full border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60"
    >
        ...
        <div class="flex items-center gap-4">
          ...
          <button
            *ngIf="showMobileMenu"
            ...
            variant="ghost"
            size="icon"
            class="h-9 w-9 rounded-md inline-flex"
          ...
          <div class="flex items-center">
            ...
            >
              ...
              <lib-logo
                *ngIf="!logoSrc"
                width="100px"
                [variant]="isDarkMode() ? 'alt' : 'default'"
              />
              <span
                *ngIf="badgeText"
                class="hidden text-xs font-bold text-muted-foreground opacity-70 sm:inline-block"
                >{{ badgeText }}</span
              >
            </a>
          </div>
        ...
        <div class="flex flex-1 items-center justify-end gap-2">
          ...
          <ng-container *ngIf="authState.isAuthenticated(); else loginBtn">
            <div class="relative">
              ...
              >
                <div
                  class="flex items-center gap-3 px-3 py-2 border-b border-border"
                >
                  <app-user-avatar
                    [src]="avatarSrc()"
                    [firstName]="getFirstName()"
                  ...
                  <div class="flex flex-col truncate">
                    ...
                    <span
                      ...
                      *ngIf="profileState.value() as user"
                      >{{ user.email }}</span
                    >
                  </div>
                </div>
          ...
          <ng-template #loginBtn>
            <button
              *ngIf="showLogin"
              hlmBtn
              variant="outline"
              class="rounded-full border-primary text-primary hover:bg-primary/5"
              (click)="onLogin()"
            >
              Entrar
            </button>
          ...
          >
            <hlm-icon
              [name]="mobileMenuOpen ? 'lucideX' : 'lucideLayoutGrid'"
            />
          </button>
        </div>
      </div>
    ...
    <!-- Mobile Drawer -->
    <div
      class="fixed top-0 bottom-0 right-0 z-[1050] w-full max-w-[300px] bg-background shadow-xl transition-transform duration-300 md:hidden"
