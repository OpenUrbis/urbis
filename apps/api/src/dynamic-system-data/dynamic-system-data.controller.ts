...
export class DynamicSystemDataController {
  ...
  async importExcel(
    ...
    if (!isExcelMimeType && !isExcelExtension) {
      throw new BadRequestException(`Arquivo inválido. Envie uma planilha Excel (.xlsx ou .xls) - Recebido: ${file.mimetype}`);
    }

    try {
      const result = await this.dynamicDataService.processExcelFile(file.buffer, version);
      return {
        message: 'Planilha processada e salva com sucesso',
        data: result
      };
    } catch (error: any) {
      throw new BadRequestException(`Erro ao processar a planilha: ${error.message}`);
    }
  }
  ...
  async getModuleData(
    ...
    const data = await this.dynamicDataService.getModuleData(moduleName, version);
    return {
      moduleName,
      version,
      data
