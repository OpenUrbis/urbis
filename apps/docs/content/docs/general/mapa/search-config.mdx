...
| `layerSchemaId`         | Opcional. Identificador de um esquema de camada (definido em `layer-schema.mdx`) usado para aplicar uma ação de clique quando um resultado de busca é selecionado. Se não fornecido, a propriedade `clickAction` é usada. | `null`        | `"lotes"`                                                               |
