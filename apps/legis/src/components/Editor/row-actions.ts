            ...
            doc.descendants((node, pos) => {
              if (node.type.name === 'tableRow') {
                ...
                if (node.firstChild) {
                    ...
                    const widget = Decoration.widget(widgetPos, (view, getPos) => {
                        const btn = document.createElement('button');
                        // Position absolute inside relative cell, visible on row hover (group-hover)
                        btn.className = 'absolute left-0.5 top-1/2 -translate-y-1/2 w-4 h-4 inline-flex items-center justify-center rounded hover:bg-primary/20 text-primary/50 hover:text-primary cursor-pointer transition-all opacity-0 group-hover:opacity-100 z-10';
                        ...
                        btn.onclick = (e) => {
                            ...
                            if (typeof getPos === 'function') {
                                ...
                                if (typeof currentPos === 'number') {
                                    ...
                                    tr.setSelection(TextSelection.near(tr.doc.resolve(currentPos)));
                                    view.dispatch(tr);
                                    view.focus();
                                }
                            }
                        ...
                        return btn;
                    }, { side: -1 }); // side -1 to appear before text
