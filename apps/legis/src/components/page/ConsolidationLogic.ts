...
export function isFutureValidity(situation: SpecialSituation): boolean {
  return (
    situation.type === "Vigência inicial alterada" ||
    situation.type === "Vigência final alterada"
  ); // Rough check
}
...
export function processElementsForConsolidated(
    ...
    if (isInactive) {
      if (!lastWasInactive) {
        ...
        result.push({
          ...
          index: "",
          specialSituations: [],
          originalStartValidity: createEmptyValidity(),
        } as any);
      }
