...
function getHierarchicalNumericParts(index?: string): number[] | null {
  if (!index) return null;

  const normalized = index.trim().replace(/\.$/, "");
...
function checkHierarchicalNumericGap(
  prevIndex?: string,
  currIndex?: string,
): boolean {
  const prevParts = getHierarchicalNumericParts(prevIndex);
  const currParts = getHierarchicalNumericParts(currIndex);

  if (!prevParts || !currParts) {
    return false;
  }

  // Mudança de profundidade costuma indicar descida/subida hierárquica normal,
  // não uma lacuna de numeração no mesmo nível.
  if (prevParts.length !== currParts.length) {
    return false;
  }

  // Só faz sentido comparar itens irmãos no mesmo ramo hierárquico.
  for (let i = 0; i < prevParts.length - 1; i++) {
    if (prevParts[i] !== currParts[i]) {
      return false;
    }
  }

  return currParts[currParts.length - 1] - prevParts[prevParts.length - 1] > 1;
}

export function checkGap(
  prev: NormativeElementEntity,
  curr: NormativeElementEntity,
): boolean {
  ...
  if (checkHierarchicalNumericGap(prev.index, curr.index)) {
    return true;
  }

  if (
    getHierarchicalNumericParts(prev.index) ||
    getHierarchicalNumericParts(curr.index)
  ) {
    return false;
  }
  ...
  for (let i = 0; i < indices.length; i++) {
    ...
    if (i < indices.length - 1) {
      ...
      if (nextIdx - currentIdx > 1) {
        result.push({
          type: "Gap",
          id: `gap-between-${fullElements[currentIdx].id}`,
        });
      }
    }
  }
  ...
  elements.forEach((el, _index) => {
    ...
    if (prevOfSameType && prevOfSameType.parentId === el.parentId) {
      if (checkGap(prevOfSameType, el)) {
        result.push({
          type: "Gap",
          id: `gap-seq-${prevOfSameType.id}-${el.id}`,
        });
      }
    }
    lastSeen[el.type] = el;
