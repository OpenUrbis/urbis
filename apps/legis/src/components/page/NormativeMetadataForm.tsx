...
function DatePartsInput({ value, onChange, disabled, maxYear, allowFuture }: DatePartsInputProps) {
        ...
        if (d && m && y && d.length > 0 && m.length > 0 && y.length === 4) {
            ...
            if (isValid(date)) {
                ...
                onChange(dateStr);
                 ...
                 // Invalid date format, but we keep typing? 
                 // Maybe clear value or pass partial?
                 // Strategy: only pass valid full dates
            }
        } else if (!d && !m && !y) {
            onChange(undefined);
        }
