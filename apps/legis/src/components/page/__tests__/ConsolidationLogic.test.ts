...
import { isElementInactive, processElementsForConsolidated } from '../ConsolidationLogic';
import { NormativeElementEntity, SpecialSituation } from '../../../domain/entities';

// Helper
const createElement = (id: string, situations: SpecialSituation[] = [], type: string = 'Artigo', text: string = 'Text'): NormativeElementEntity => ({
    id,
    type: type as any,
    index: '1',
    text,
...
describe('ConsolidationLogic', () => {
    describe('isElementInactive', () => {
        ...
        it('should be inactive if Revoked', () => {
            const el = createElement('1', [{ type: 'Revogação', date: '01.01.2021' }]);
