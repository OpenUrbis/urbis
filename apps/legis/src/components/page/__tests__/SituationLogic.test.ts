...
const createElement = (
  ...
  originalStartValidity: validity.start || {
    date: "01.01.2020",
    deviceId: "Lei 1",
  },
...
describe("SituationLogic", () => {
  describe("groupSituationsForElement", () => {
    ...
    it("should order situations chronologically (oldest first, newest last)", () => {
      const el = createElement([
        { type: "Veto", date: "01.01.2021" },
      ...
      expect(dates).toEqual(["01.01.2021", "01.02.2021", "01.03.2021"]);
    });
  ...
  describe("getElementStyle", () => {
    ...
    it("should apply Orange for Derrubada alone", () => {
      const el = createElement([
        { type: "Derrubada de veto", date: "01.01.2022" },
      ]);
      expect(getElementStyle(el)).toEqual({ color: "orange" });
    });
    ...
    it("should apply Red for Future Validity", () => {
      ...
      const el = createElement([
        { type: "Vigência inicial alterada", date: dateStr },
      ]);
    ...
    it("should apply Underline for Interpretation", () => {
      const el = createElement([
        { type: "Interpretação conforme à Constituição", date: "01.01.2022" },
      ]);
  ...
  describe("Validity Checks", () => {
    ...
    it("isValidityEnded returns true for past end date", () => {
      const el = createElement([], {
        end: { date: "01.01.2000", deviceId: "Lei 1" },
      }); // Original end validity
      expect(isValidityEnded(el, { date: "01.01.2000" })).toBe(true);
    });
  });

  describe("special situation compatibility helpers", () => {
    it("prefers dispositivo and relatedDeviceId when present", () => {
      ...
      expect(getSituationDeviceLabel(situation)).toBe("Art. 10");
      expect(getSituationRelatedDeviceId(situation)).toBe("art-10");
    });

    it("falls back to legacy device and normativeElementId fields", () => {
      const situation: SpecialSituation = {
      ...
      expect(getSituationDeviceLabel(situation)).toBe("Ementa");
      expect(getSituationRelatedDeviceId(situation)).toBe("ementa-1");
    });

    it("normalizes compatibility fields for legacy and new consumers", () => {
      const normalized = withSpecialSituationCompatibilityFields({
        type: "Acréscimo",
        device: "§ 1º",
        normativeElementId: "par-1",
        newText: "Texto acrescido",
      });

      expect(normalized.dispositivo).toBe("§ 1º");
      expect(normalized.device).toBe("§ 1º");
      expect(normalized.relatedDeviceId).toBe("par-1");
      expect(normalized.normativeElementId).toBe("par-1");
    });

    it("identifies new text situations and returns display labels", () => {
      expect(isNewTextSituationType("Repristinação")).toBe(true);
      expect(
        isNewTextSituation({ type: "Interpretação conforme à Constituição" }),
      ).toBe(true);
      expect(isNewTextSituation({ type: "Veto" })).toBe(false);
      expect(
        getSituationNewTextLabel(
          "Declaração de inconstitucionalidade sem redução de texto",
        ),
      ).toBe("Texto com declaração de inconstitucionalidade");
    });
  });
});
