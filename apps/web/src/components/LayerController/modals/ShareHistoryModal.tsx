...
import {
  shareService,
  SharedMapItem,
} from "../../../integrations/share-service";
...
export const ShareHistoryModal = ({
  ...
  const copyLink = (item: SharedMapItem) => {
    void navigator.clipboard.writeText(
      `${window.location.origin}/?shareId=${item.id}`,
    );
  };

  const deleteItem = async (item: SharedMapItem) => {
    const confirmed = window.confirm(`Excluir "${item.name}"?`);
    if (!confirmed) return;

    await shareService.delete(item.id);
    history.value = history.value.filter((current) => current.id !== item.id);
          ...
          {loading.value ? (
            ...
            <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-1">
              {history.value.map((item) => (
                <div
                  key={item.id}
                  className="group border rounded-lg p-3 flex justify-between items-center bg-card hover:bg-accent/50 transition-all cursor-pointer"
