...
export const MapView = ({
  ...
  const layers = computed(() => {
    ...
    const inspectionPointLayers =
      polygonEdit.feature.value?.geometry?.type === "Point"
        ? [
            new GeoJsonLayer({
              id: "inspection-point-marker-layer",
              data: polygonEdit.feature.value,
              pointType: "circle",
              filled: true,
              stroked: true,
              getFillColor: [234, 67, 53, 230],
    ...
    const highlightTarget = rawHighlight
      ? normalizeGeoJsonToWgs84(rawHighlight)
      : null;
  ...
  const createMicroPointFeature = (lat: number, lon: number) => {
    const sideMeters = 0.01; // 1 cm micro-geometry for point query
    const halfMeters = sideMeters / 2;
    const deltaLat = halfMeters / 111320;
    const deltaLon =
      halfMeters / (111320 * Math.cos((lat * Math.PI) / 180));
  ...
  const panToLocationWithZoomCheck = (lat: number, lon: number) => {
    const currentZoom = zoom.value || 10;
    const targetZoom = currentZoom < 15 ? 16.5 : currentZoom;
    if (mapFlyTo) {
      mapFlyTo({
        center: [lon, lat],
        zoom: targetZoom,
  ...
  const handleInspectAttributesHere = (lat: number, lon: number) => {
    setContextMenu(null);

    // Pan and zoom into location if zoom is below 15
    panToLocationWithZoomCheck(lat, lon);

    // Point inspection (clean marker point, not drawing mode)
    const pointFeature = {
      type: "Feature" as const,
      id: `point-inspection-${Date.now()}`,
      properties: {
        layer: "Ponto Consultado",
        latitude: lat,
        longitude: lon,
        isPointInspection: true,
      },
    ...
    const microQueryPolygon = createMicroPointFeature(lat, lon);

    polygonEdit.reset();
    polygonEdit.setFeature(pointFeature);
    polygonEdit.setIsEditing(false);
    polygonEdit.drawRef.current?.deleteAll();

    polygonEdit.fetchData(microQueryPolygon);
  };
  ...
  useEffect(() => {
    const handleOpenDigitalAddressEvent = (event: Event) => {
      const customEvt = event as CustomEvent<{
        latitude: number;
        longitude: number;
      }>;
      if (customEvt.detail?.latitude && customEvt.detail?.longitude) {
        openDigitalAddressAt(
          customEvt.detail.latitude,
          customEvt.detail.longitude,
        );
      }
    };

    const handleInspectAttributesEvent = (event: Event) => {
      const customEvt = event as CustomEvent<{
        latitude: number;
        longitude: number;
      }>;
      if (customEvt.detail?.latitude && customEvt.detail?.longitude) {
        handleInspectAttributesHere(
          customEvt.detail.latitude,
          customEvt.detail.longitude,
        );
      }
    ...
    window.addEventListener(
      ...
      handleOpenDigitalAddressEvent as EventListener,
    );
    window.addEventListener(
      "inspect-map-attributes",
      handleInspectAttributesEvent as EventListener,
    );
    return () => {
  ...
  }, [features.digitalAddress, isDesktop, drawerOpen.value]);
  ...
  const handleClick = (info: PickingInfo) => {
    ...
    if (isPickingLocation.value) {
      if (info.coordinate) {
        ...
        handleInspectAttributesHere(lat, lon);
        isPickingLocation.value = false;
      }
      return;
    }
    ...
    // If Admin explicitly disabled click action on this layer
    if (clickAction && (clickAction.action === "none" || (clickAction as any) === "none")) {
      return;
    }

    if (info?.object && (info.object.id || info.object.properties)) {
      polygonEdit.reset();
      polygonEdit.setFeature(null);
      polygonEdit.drawRef.current?.deleteAll();
  ...
  const saveButton = () => {
    ...
    const analysisDisabled = loading || !hasDrawnFeature;
    ...
    return (
      <div className="urbis-draw-layer absolute bottom-24 right-4 z-[999999] flex items-center gap-2 animate-in fade-in slide-in-from-bottom-2 pointer-events-auto">
        ...
        <Button
          ...
          variant="secondary"
          size="sm"
          title="Criar camada permanente a partir do desenho"
          ...
          <div className="space-y-0.5 p-0.5">
            ...
            >
              <span className="flex items-center gap-2 text-foreground">
                <TableProperties className="h-3.5 w-3.5 shrink-0 text-primary" />
                Consultar tabela de atributos aqui
              </span>
              <InfoTooltip>
                Abre a tabela de atributos e parâmetros urbanísticos do ponto ou geometria selecionada.
              </InfoTooltip>
            </button>
            <button
              type="button"
      ...
      >
        {viewport.value ? (
          <Map
            ...
            attributionControl={minimalPreview ? false : undefined}
            boxZoom={true}
            dragPan={true}
