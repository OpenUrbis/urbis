...
export function transformBoundsToUTM(coordinates: any) {
    ...
    const destination = '+proj=utm +zone=23 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs';
...
export function transformUTMToWGS84(coordinates: any) {
    ...
    const source = '+proj=utm +zone=23 +south +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs';
...
export function convertGeoJsonCoordinates(geoJson: any): any {
    ...
    const convertedFeatures = geoJson.features.map((feature: any) => {
        const convertedCoordinates = feature.geometry.coordinates.map((coordinates: any) => {
            if (feature.geometry.type === 'Polygon') {
                return coordinates.map((ring: any) => ring.map((coord: number[]) => proj4(source, destination, coord)));
            } else if (feature.geometry.type === 'MultiPolygon') {
                return coordinates.map((polygon: any) => polygon.map((ring: any) => ring.map((coord: number[]) => proj4(source, destination, coord))));
            }
