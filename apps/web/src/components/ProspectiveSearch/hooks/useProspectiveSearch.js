...
export function isNAValue(val) {
    if (val === undefined || val === null)
        return true;
    const s = String(val).trim().toUpperCase();
    if (!s || s === "-")
        return true;
    return (s.startsWith("NA") ||
        s.startsWith("ND") ||
        s.startsWith("N.D.") ||
        s.startsWith("N.A."));
}
export function useProspectiveSearch(moduleData) {
    // --- Data References ---
    ...
    const useSearchResults = useMemo(() => {
        if (debouncedUseSearchTerm.length < 2)
            return null;
        if (searchMode === "cnpj")
            return null; // CNPJ search is handled asynchronously in the UI
        return pesquisarUsos(debouncedUseSearchTerm, usosData, cnaeData, searchMode);
    ...
    // --- Actions ---
    const updateUrbanParam = (param, value) => {
        setUrbanParams((prev) => ({
            ...prev,
            [param]: value,
        }));
    };
    const removeUrbanParam = (param) => {
        setUrbanParams((prev) => {
            const next = { ...prev };
            delete next[param];
            return next;
    ...
    const updatePqaParam = (param, value) => {
        setPqaParams((prev) => ({
            ...prev,
            [param]: value,
        }));
    };
    const removePqaParam = (param) => {
        setPqaParams((prev) => {
            const next = { ...prev };
            delete next[param];
            return next;
    ...
    const isParamEnabledForArea = (paramId, areaRange) => {
        let requiredMin = 0;
        ...
        if (normalizedParamId.includes("até 500")) {
            ...
            while ((match = condRegex.exec(normalizedParamId)) !== null) {
                ...
                if (op === ">")
                    requiredMin = Math.max(requiredMin, val + 0.01);
                else if (op === "≥")
                    requiredMin = Math.max(requiredMin, val);
                else if (op === "<")
                    requiredMax = Math.min(requiredMax, val - 0.01);
                else if (op === "≤")
                    requiredMax = Math.min(requiredMax, val);
            }
        }
        if (!hasAreaCondition)
            return true;
    ...
    const updateAreaImovel = (newArea) => {
    ...
    const selectedUseCode = useMemo(() => {
        ...
        const rawCode = getVal(selectedUse?.arvore.tipologiaOuGrupo, [
            "Código",
            "Codigo",
            "codigo",
        ]) ||
        ...
        if (rawCode.startsWith("HIS1-"))
            return "HIS1";
        if (rawCode.startsWith("HIS2-"))
            return "HIS2";
        if (rawCode.startsWith("HMP-"))
            return "HMP";
        return rawCode;
    }, [selectedUse]);
    ...
    const matchingZones = useMemo(() => {
        const paramZoneNames = urbanParamsResults.map((r) => r["zona"] || r["Zona"] || Object.values(r)[0]);
    ...
    const compatiblePqas = useMemo(() => {
        const pqaNames = urbanParamsPqaResults.map((r) => r["perimetroQualificacaoAmbiental"] ||
            r["perimetro"] ||
            r["Perímetro"] ||
            Object.values(r)[0]);
        ...
        if (Object.keys(pqaParams).length === 0 && !selectedUseCode)
            return [];
        return Array.from(new Set(pqaNames)).sort();
    }, [urbanParamsPqaResults, pqaParams, selectedUseCode]);
    const allPqaNames = useMemo(() => {
        return Array.from(new Set(parametrosPqaData.map((r) => r["perimetroQualificacaoAmbiental"] ||
            r["perimetro"] ||
            r["Perímetro"] ||
            Object.values(r)[0]))).sort();
    }, [parametrosPqaData]);
