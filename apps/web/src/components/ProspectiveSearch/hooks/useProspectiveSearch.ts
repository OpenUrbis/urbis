...
export function isNAValue(val: any): boolean {
...
export function useProspectiveSearch(moduleData: Record<string, any[]>) {
  ...
  const cnaeData = useMemo(
    () => moduleData["CNAEs por Atividade"] || [],
    [moduleData],
  );
  const cnaeCatalogData = useMemo(() => moduleData["CNAE"] || [], [moduleData]);

  // You might need these in the UI for cross-referencing
  const usosPorZonaData = useMemo(
    () => moduleData["Usos permitidos por Zona"] || [],
    [moduleData],
  );
  const parametrosUsoData = useMemo(
    () => moduleData["Parâmetros urbanísticos por Uso"] || [],
    [moduleData],
  );
  const parametrosZonasData = useMemo(() => {
    // Fuzzy match for the sheet name to avoid truncation issues
    const key = Object.keys(moduleData).find((k) =>
      k.startsWith("Parâmetros urbanísticos por Zon"),
    );
  ...
  const parametrosPqaData = useMemo(() => {
    const key = Object.keys(moduleData).find((k) =>
      k.startsWith("Parâmetros urbanísticos por Per"),
    );
    return key ? moduleData[key] : [];
  }, [moduleData]);
  ...
  const [selectedUse, setSelectedUse] = useState<UsoSearchResultItem | null>(
    null,
  );
  ...
  const availableOptions = useMemo(() => {
    ...
    const KEYS_TO_EXTRACT: Record<string, string[]> = {
      ...
      "Coeficiente de Aproveitamento Mínimo": [
        "coeficienteAproveitamentoMinimo",
      ],
      "Coeficiente de Aproveitamento Básico": [
        "coeficienteAproveitamentoBasico",
      ],
      "Coeficiente de Aproveitamento Máximo": [
        "coeficienteAproveitamentoMaximo",
      ],
      "Taxa de Ocupação Máxima para Lotes até 500,00 m²": [
        "taxaOcupacaoMaximaLotesAte500",
      ],
      "Taxa de Ocupação Máxima para Lotes igual ou superior a 500,00 m²": [
        "taxaOcupacaoMaximaLotesIgualOuSuperior500",
      ],
      "Gabarito de Altura Máxima (m)": ["gabaritoAlturaMaxima"],
      "Recuo Mínimo de Frente (m)": ["recuoMinimoFrente"],
      "Recuo Mínimo de Fundos e Laterais (H ≤ 10m)": [
        "recuoMinimoFundosLateraisAlturaEdificacaoMenorOuIgual10",
      ],
      "Recuo Mínimo de Fundos e Laterais (H > 10m)": [
        "recuoMinimoFundosLateraisAlturaEdificacaoSuperior10",
      ],
      "Cota Parte Máxima de Terreno por Unidade (m²)": [
        "cotaParteMaximaTerrenoPorUnidade",
      ],
      "Ruído (7h-19h)": [
        "nivelCriterioAvaliacaoNcaAmbienteExternoDbEmissaoRuido7hAs19h",
      ],
      "Ruído (19h-22h)": [
        "nivelCriterioAvaliacaoNcaAmbienteExternoDbEmissaoRuido19hAs22h",
      ],
      "Ruído (22h-7h)": [
        "nivelCriterioAvaliacaoNcaAmbienteExternoDbEmissaoRuido22hAs7h",
      ],
  ...
  const pqaOptions = useMemo(() => {
    ...
    const KEYS_TO_EXTRACT: Record<string, string[]> = {
      ...
      "Taxa de Permeabilidade Lote ≤ 500": ["taxaPermeabilidadeLote≤500"],
      "Pontuação QA Mínimo Lote > 500 ≤ 1000": [
        "pontuacaoQaMinimoLote>500≤1000",
      ],
      "Pontuação QA Mínimo Lote > 1000 ≤ 2500": [
        "pontuacaoQaMinimoLote>1000≤2500",
      ],
      "Pontuação QA Mínimo Lote > 2500 ≤ 5000": [
        "pontuacaoQaMinimoLote>2500≤5000",
      ],
      "Pontuação QA Mínimo Lote > 5000 ≤ 10000": [
        "pontuacaoQaMinimoLote>5000≤10000",
      ],
      "Fator Incentivo Quota Ambiental (Lote > 5000, Pontuação ≥ 4x Qamin)": [
        "fatorIncentivoQuotaAmbientalR$/Lote>5.000Pontuacao≥4VezesQamin",
      ],
      "Fator Incentivo Quota Ambiental (Lote > 5000, Pontuação ≥ 2x e < 3x Qamin)":
        ["fatorIncentivoQuotaAmbientalR$/Lote>5.000Pontuacao≥2<3VezesQamin"],
      "Fator Incentivo Quota Ambiental (Lote > 5000, Pontuação ≥ 3x e < 4x Qamin)":
        ["fatorIncentivoQuotaAmbientalR$/Lote>5.000Pontuacao≥3<4VezesQamin"],
      "Fator Incentivo Quota Ambiental (Lote > 5000, Pontuação ≥ 1.5x e < 2x Qamin)":
        ["fatorIncentivoQuotaAmbientalR$/Lote>5.000Pontuacao≥1,5<2VezesQamin"],
      "Fator Incentivo Quota Ambiental (Lote > 500 ≤ 1000, Pontuação ≥ 4x Qamin)":
        ["fatorIncentivoQuotaAmbientalR$/Lote>500≤1.000Pontuacao≥4VezesQamin"],
      "Fator Incentivo Quota Ambiental (Lote > 1000 ≤ 2500, Pontuação ≥ 4x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>1.000≤2.500Pontuacao≥4VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 2500 ≤ 5000, Pontuação ≥ 4x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>2.500≤5.000Pontuacao≥4VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 500 ≤ 1000, Pontuação ≥ 2x e < 3x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>500≤1.000Pontuacao≥2<3VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 500 ≤ 1000, Pontuação ≥ 3x e < 4x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>500≤1.000Pontuacao≥3<4VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 1000 ≤ 2500, Pontuação ≥ 2x e < 3x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>1.000≤2.500Pontuacao≥2<3VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 1000 ≤ 2500, Pontuação ≥ 3x e < 4x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>1.000≤2.500Pontuacao≥3<4VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 2500 ≤ 5000, Pontuação ≥ 2x e < 3x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>2.500≤5.000Pontuacao≥2<3VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 2500 ≤ 5000, Pontuação ≥ 3x e < 4x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>2.500≤5.000Pontuacao≥3<4VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 500 ≤ 1000, Pontuação ≥ 1.5x e < 2x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>500≤1.000Pontuacao≥1,5<2VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 1000 ≤ 2500, Pontuação ≥ 1.5x e < 2x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>1.000≤2.500Pontuacao≥1,5<2VezesQamin",
        ],
      "Fator Incentivo Quota Ambiental (Lote > 2500 ≤ 5000, Pontuação ≥ 1.5x e < 2x Qamin)":
        [
          "fatorIncentivoQuotaAmbientalR$/Lote>2.500≤5.000Pontuacao≥1,5<2VezesQamin",
        ],
    };
  ...
  const debouncedUseSearchTerm = useDebouncedValue(useSearchTerm, 350);
  const debouncedUrbanParams = useDebouncedValue(urbanParams, 250);
  const debouncedPqaParams = useDebouncedValue(pqaParams, 250);

  // --- Derived Data (Search Results) ---

  const useSearchResults: PesquisaUsoResult | null = useMemo(() => {
    if (debouncedUseSearchTerm.length < 2) return null;
    if (searchMode === "cnpj") return null; // CNPJ search is handled asynchronously in the UI
    return pesquisarUsos(
      debouncedUseSearchTerm,
      usosData,
      cnaeData,
      searchMode as "atividade" | "cnae",
    );
  }, [debouncedUseSearchTerm, usosData, cnaeData, searchMode]);
  ...
  const urbanParamsResults = useMemo(() => {
    ...
    Object.keys(debouncedUrbanParams).forEach((paramKey) => {
      const filter = debouncedUrbanParams[paramKey];
      ...
      filteredZonas = filteredZonas.filter((zonaData) => {
        ...
        if (value === null) return !!filter.includeNA;

        switch (filter.type) {
          case "between": {
            const minValue = Number.isFinite(filter.min)
              ? filter.min
              : -Infinity;
            const maxValue = Number.isFinite(filter.max)
              ? filter.max
              : Infinity;
            return value >= minValue && value <= maxValue;
          }
          case "greater_than":
            return Number.isFinite(filter.value) ? value > filter.value : true;
          case "less_than":
            return Number.isFinite(filter.value) ? value < filter.value : true;
          case "equal":
  ...
  }, [debouncedUrbanParams, parametrosZonasData]);
  ...
  const urbanParamsPqaResults = useMemo(() => {
    ...
    Object.keys(debouncedPqaParams).forEach((paramKey) => {
      const filter = debouncedPqaParams[paramKey];
      const dataKey = PARAM_KEY_MAPPING[paramKey] || paramKey;
    ...
    return filteredPqas;
  }, [debouncedPqaParams, parametrosPqaData]);
  ...
  const isParamEnabledForArea = (
    ...
    if (areaRange === null || areaRange === undefined) return true;
  ...
  const selectedUseCode = useMemo(() => {
    ...
    // Mapeamento de subtipologias e tipologias de HIS e HMP para seus respectivos códigos base,
    // uma vez que as regras de zoneamento e parâmetros estão cadastradas apenas sob o código genérico.
  ...
  const matchingZones = useMemo(() => {
    const paramZoneNames = urbanParamsResults.map(
      (r: any) => r["zona"] || r["Zona"] || Object.values(r)[0],
    );
  ...
  const compatiblePqas = useMemo(() => {
    const pqaNames = urbanParamsPqaResults.map(
      (r: any) =>
        r["perimetroQualificacaoAmbiental"] ||
        r["perimetro"] ||
        r["Perímetro"] ||
        Object.values(r)[0],
    );
