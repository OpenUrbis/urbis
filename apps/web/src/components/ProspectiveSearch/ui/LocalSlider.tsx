...
interface LocalSliderProps extends React.ComponentPropsWithoutRef<
...
>(
  (
    { className, ticks, tickLabels, min = 0, max = 100, step = 1, ...props },
    ref,
  ) => {
            ...
            <div className="absolute inset-0 pointer-events-none">
              {Array.from({ length: ticks }).map((_, i) => {
                ...
                const getValPercentage = (val: number) => {
                  const sMin = min ?? 0;
                  const sMax = max ?? 100;
                  if (!Number.isFinite(val) || sMax === sMin) return 0;
                ...
                const isSelected = Array.isArray(value)
                  ? value.length === 2
                    ...
                    : Math.abs(tickPercentage - getValPercentage(value[0])) <
                      0.5
                  : Math.abs(
                      tickPercentage - getValPercentage(value as number),
                    ) < 0.5;
