 ...
 */
export const extractNotas = (valorRaw) => {
    const notas = [];
 ...
 */
export const buildUsoHierarchy = (codigo, todosUsos) => {
    ...
    if (normalizedDiv === "atividade" || normalizedDiv === "subtipologia") {
        ...
        if (parentCode) {
            arvore.tipologiaOuGrupo = todosUsos.find((u) => getVal(u, ["Código", "Codigo", "codigo"]) === parentCode);
        }
    }
    else if (normalizedDiv === "tipologia" ||
    ...
    if (!arvore.subcategoria && arvore.tipologiaOuGrupo) {
        ...
        if (targetCode)
            arvore.subcategoria = todosUsos.find((u) => getVal(u, ["Código", "Codigo", "codigo"]) === targetCode);
    }
    if (!arvore.categoria && arvore.subcategoria) {
        ...
        if (targetCode)
            arvore.categoria = todosUsos.find((u) => getVal(u, ["Código", "Codigo", "codigo"]) === targetCode);
    }
    return arvore;
};
/**
 ...
 */
export const pesquisarUsos = (termo, usosData, cnaeData, modo = "atividade") => {
    const result = {
        tipoBusca: "TEXTO",
        termoBuscado: termo,
        resultados: [],
    };
    if (!termo || !usosData || !usosData.length)
        return result;
    ...
    if (modo === "atividade") {
        ...
        const usosPorCodigo = usosData.filter((u) => {
            ...
            return (codClean.includes(termoCodigoClean) ||
    ...
    if (modo === "cnae" && cnaeData && cnaeData.length) {
        ...
        if (cnaesEncontrados.length > 0) {
            ...
            // Remover duplicatas
            result.resultados = result.resultados.filter((v, i, a) => a.findIndex((t) => getVal(t.item, ["Código", "Codigo", "codigo"]) ===
                getVal(v.item, ["Código", "Codigo", "codigo"])) === i);
            if (result.resultados.length > 0)
                return result;
        }
    }
    ...
    if (modo === "atividade") {
        ...
        const usosEncontrados = usosData.filter((u) => {
            ...
            if ((div === "categoria de uso" || div === "subcategoria de uso") &&
                cod !== "r1") {
                return false;
            }
            const compactedSearchIndex = `${cod} ${div} ${desc}`.toLowerCase();
            const compactedSearchIndexClean = compactedSearchIndex.replace(/[^a-z0-9]/g, "");
            return (normalizeString(desc).includes(termoNormalizado) ||
 ...
 */
export const getRegrasZonamento = (codigoTipologiaOuGrupo, usosPorZonaData) => {
    if (!usosPorZonaData || !usosPorZonaData.length)
        return null;
    const possibleKeys = [
    ...
    const keyIdentifier = Object.keys(usosPorZonaData[0]).find((k) => possibleKeys.includes(k.toLowerCase().trim()) ||
        k.toLowerCase().includes("tipologia") ||
        k.toLowerCase().includes("grupo")) || Object.keys(usosPorZonaData[0])[0];
    // Safe string compare to ensure we match codes like "R1" to "R1" safely
    const regra = usosPorZonaData.find((r) => r[keyIdentifier] &&
    ...
    Object.keys(regra).forEach((zona) => {
        ...
        const isNao = valor.startsWith("N") ||
            valor.startsWith("NAO") ||
            valor.startsWith("NÃO");
        if (isSim && !hasNota)
            resultadoFormatado.simSemNota.push(zona);
        else if (isNao && !hasNota)
            resultadoFormatado.naoSemNota.push(zona);
 ...
 */
export const getCondicoesInstalacao = (codigoTipologiaOuGrupo, parametrosUsoData) => {
    if (!parametrosUsoData || !parametrosUsoData.length)
        return [];
    const possibleKeys = [
        "subcategoriaUsoGruposAtividadeUsosEspecificos",
        "subcategoria de uso, grupos de atividade e usos específicos",
    ];
    const keyIdentifier = Object.keys(parametrosUsoData[0]).find((k) => possibleKeys.includes(k.toLowerCase().trim()) ||
        k.toLowerCase().includes("tipologia") ||
        k.toLowerCase().includes("grupo")) || Object.keys(parametrosUsoData[0])[0];
    const regra = parametrosUsoData.find((r) => r[keyIdentifier] &&
        String(r[keyIdentifier]).trim().toLowerCase() ===
            String(codigoTipologiaOuGrupo).trim().toLowerCase());
    if (!regra)
        return [];
    ...
    Object.keys(regra).forEach((parametro) => {
        if (parametro === keyIdentifier || !parametro)
            return;
