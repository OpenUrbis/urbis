...
const BuilderContext = createContext<IBuilderContextType | undefined>(
  undefined,
);
...
const DEFAULT_MOCK_DATA = {
  ...
  bbox: [-46.6377824188, -23.5363474093, -46.6375660449, -23.5360541954],
};
  ...
  const moveItem = useCallback(
    (dragId: string, overId: string, newIndex?: number) => {
      setTemplate((prev) => {
        ...
        const treeWithoutDragged = extractItem(
        ...
        const itemToModify = draggedItem as any;
        ...
        if (newParentId === null) {
          ...
          if (!dropTargetIsContainer && indexToInsert === undefined) {
            ...
            const siblingIdx = findSiblingIndex(
              treeWithoutDragged,
              newParentId,
            );
          ...
          const finalTree = treeWithoutDragged.map((item) =>
            addNode(item, newParentId, draggedItem as ITemplate, indexToInsert),
          );
