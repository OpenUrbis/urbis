...
const ConfigForm = ({
  ...
  const defaultFillColor = extractColorArray(
    properties.polygonProps || "",
    "getFillColor",
    [30, 111, 249, 0.4],
  );
  const defaultLineColor = extractColorArray(
    properties.polygonProps || "",
    "getLineColor",
    [30, 111, 249, 1],
  );
  ...
  const { register, watch, setValue } = useForm({
    defaultValues: {
      ...
      // Simple Mode state
      autoCenter:
        properties.initialViewState?.includes("calculateCenterId") ?? true,
    },
  });
  ...
  const applySimpleSettings = (
    currentColor: ILayerColor,
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b pb-4">
        ...
        >
          Configuração Avançada (Funções JS)
        </Label>
        <input
          type="checkbox"
