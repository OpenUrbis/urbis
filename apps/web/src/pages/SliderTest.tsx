...
const CustomSlider = React.forwardRef<HTMLDivElement, any>(({ min = 0, max = 100, step = 1, value, defaultValue, onValueChange, className, ...props }, ref) => {
  // Handle both controlled and uncontrolled modes (simplistic)
  const [internalValue, setInternalValue] = useState(value || defaultValue || [min, max]);
...
export default function SliderTestPage() {
  ...
  return (
    <div className="p-10 w-full h-full bg-white text-black overflow-auto" style={{ zIndex: 9999, position: 'relative' }}>
      <h1 className="text-2xl mb-4 font-bold">Slider Debug Reproduction (Custom Slider)</h1>
