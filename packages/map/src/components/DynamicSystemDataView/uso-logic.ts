...
export const buildUsoHierarchy = (
  ...
  if (item.Divisão === "Atividade" || item.Divisão === "Subtipologia") {
    ...
    if (item["Grupo de atividades ou Tipologia"]) {
      arvore.tipologiaOuGrupo = todosUsos.find(
        (u) => u.Código === item["Grupo de atividades ou Tipologia"],
      );
    }
  } else if (
    item.Divisão === "Tipologia" ||
    item.Divisão === "Grupo de atividades"
  ) {
  ...
  if (
    !arvore.subcategoria &&
    arvore.tipologiaOuGrupo &&
    arvore.tipologiaOuGrupo["Subcategoria de uso"]
  ) {
    arvore.subcategoria = todosUsos.find(
      (u) => u.Código === arvore.tipologiaOuGrupo["Subcategoria de uso"],
    );
  }
  ...
  if (
    !arvore.categoria &&
    arvore.subcategoria &&
    arvore.subcategoria["Categoria de uso"]
  ) {
    arvore.categoria = todosUsos.find(
      (u) => u.Código === arvore.subcategoria["Categoria de uso"],
    );
  }
...
export const pesquisarUsos = (
  ...
  if (!termo.includes(" ")) {
    const usoPorCodigo = usosData.find((u) => {
      const cod = u.Código ? u.Código.trim().toLowerCase() : "";
      const div = u.Divisão ? u.Divisão.trim().toLowerCase() : "";
      return (
        cod === termo.trim().toLowerCase() &&
  ...
  if (isPossivelCnae && cnaeData && cnaeData.length) {
    const cnaesEncontrados = cnaeData.filter(
      (c) =>
    ...
    if (cnaesEncontrados.length > 0) {
      ...
      result.resultados = result.resultados.filter(
        (v, i, a) => a.findIndex((t) => t.item.Código === v.item.Código) === i,
      );
  ...
  const usosEncontrados = usosData.filter((u) => {
    ...
    return (
      ...
      compactedSearchIndex.includes(termo.trim().toLowerCase()) ||
...
export const getRegrasZonamento = (
  ...
  const regra = usosPorZonaData.find(
    (r) => r[keyIdentifier] === codigoTipologiaOuGrupo,
  );
