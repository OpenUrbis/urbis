            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b bg-muted/40 text-muted-foreground font-semibold">
                  <th className="py-2 px-3 w-[40%]">Atributo</th>
                  <th className="py-2 px-3 w-[52%]">Valor</th>
                  <th className="py-2 px-3 w-[8%] text-center">Copiar</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border/60">
                {filteredEntries.map(([key, value]) => {
                  const label = formatAttributeLabel(key);
                  const { display, isUrl, isNumeric } = formatAttributeValue(value, key);
                  const isCopied = copiedKey === key;

                  return (
                    <tr key={key} className="hover:bg-muted/30 transition-colors group">
                      <td className="py-2 px-3 font-medium text-foreground/90 align-top">
                      ...
                      <td className={cn("py-2 px-3 text-foreground break-words align-top", isNumeric && "font-mono")}>
                        {isUrl ? (
                          <a
                            href={display}
                            target="_blank"
                      ...
                      <td className="py-2 px-3 text-center align-top">
                        <button
                          ...
                          className={cn(
                            "p-1 rounded transition-colors text-muted-foreground hover:text-foreground hover:bg-muted opacity-0 group-hover:opacity-100 focus:opacity-100",
                            isCopied && "text-emerald-600 opacity-100"
                          )}
                          title="Copiar valor"
