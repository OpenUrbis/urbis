...
export const buildLayerVisualFormDefaults = (
  ...
  return {
    ...parsed,
    url: parsed.url,
    loadingMethod: parsed.loadingMethod || "CustomWMSLayer",
    version: parsed.version || "1.3.0",
    srs: parsed.srs || "EPSG:3857",
    groupId: parsed.groupId || "geral",
    layerName: parsed.layerName || layer.name,
    minZoom: parsed.minZoom || "",
    maxZoom: parsed.maxZoom || "",
    clickAction: parsed.clickAction || "none",
    isActive: parsed.isActive ?? true,
    isSelected: parsed.isSelected ?? false,
    isVisible: parsed.isVisible ?? true,
    isDynamic: parsed.isDynamic ?? false,
    lineWidth: parsed.lineWidth ?? DEFAULT_LAYER_LINE_WIDTH,
    label: parsed.label || {
      enabled: false,
      property: "",
      minZoom: "",
    ...
    colors: parsed.colors?.length ? parsed.colors : [DEFAULT_LAYER_FORM_COLOR],
    selectedLayer: parsed.selectedLayer || {
      name: layer.name,
      title: layer.name,
    },
  };
