...
const utm23s = "+proj=utm +zone=23 +south +ellps=WGS84 +datum=WGS84 +units=m +no_defs";
const wgs84 = "EPSG:4326";

/**
 * Converte coordenadas UTM (Easting, Northing) para [Longitude, Latitude]
 */
export function utmToLatLng(easting: number, northing: number): [number, number] {
  try {
...
export function convertGeometryToWGS84(geometry: any): any {
  ...
  if (!raw.coordinates || !Array.isArray(raw.coordinates)) {
    console.error('convertGeometryToWGS84: Geometria sem coordenadas válidas', raw);
    return raw;
  }
  ...
  console.log('convertGeometryToWGS84: Exemplo de coordenada convertida:', 
    raw.coordinates[0][0], ' -> ', result.coordinates[0][0]);
