...
const MapPickerContent = ({
  ...
  const {
    ...
    data: intersections,
    loading,
    setFeature: setEditFeature,
    fetchData,
    editFeature: startEditing,
    reset: resetPolygonEdit,
  ...
  useEffect(() => {
    ...
    if (initialData) {
      if (initialData.selectedFeatures)
        selectedFeatures.value = initialData.selectedFeatures;
      if (initialData.digitalAddress)
        digitalAddressFeature.value = initialData.digitalAddress;
  ...
  const handleMapLoad = () => {
    ...
    if (initialData?.editFeature) {
      ...
      if (initialData.editFeature.geometry?.coordinates) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const centroid: any = calculateCentroid(
          initialData.editFeature.geometry.coordinates,
        );
        flyTo({
        ...
        {mode === "editable" && (
          <div className="absolute top-4 left-2 z-10 flex max-w-[420px] flex-col gap-2 overflow-y-auto pointer-events-auto scrollbar-thin">
            {!hideMap && (
              <>
                <div
                  style={{ display: isPickerSidebarOpen ? "block" : "none" }}
                >
                  <Search
