...
export const buildUsoHierarchy = (
  ...
  const item = todosUsos.find(u => getVal(u, ['Código', 'Codigo', 'codigo']) === codigo);
  if (!item) return {};

  const arvore: UsoHierarchy = {};
  const div = getVal(item, ['Divisão', 'Divisao', 'divisao', 'Divisão ']);

  if (div === 'Atividade' || div === 'Subtipologia') {
    arvore.subtipologiaOuAtividade = item;
    const parentCode = getVal(item, ['Grupo de atividades ou Tipologia', 'grupoAtividadesOuTipologia']);
    if (parentCode) {
      arvore.tipologiaOuGrupo = todosUsos.find(u => getVal(u, ['Código', 'Codigo']) === parentCode);
    }
  } else if (div === 'Tipologia' || div === 'Grupo de atividades') {
    arvore.tipologiaOuGrupo = item;
  } else if (div === 'Subcategoria de uso') {
    arvore.subcategoria = item;
  } else if (div === 'Categoria de uso') {
  ...
  if (!arvore.subcategoria && arvore.tipologiaOuGrupo) {
    const targetCode = getVal(arvore.tipologiaOuGrupo, ['Subcategoria de uso', 'subcategoriaUso']);
    if (targetCode) arvore.subcategoria = todosUsos.find(u => getVal(u, ['Código', 'Codigo', 'codigo']) === targetCode);
  }

  if (!arvore.categoria && arvore.subcategoria) {
    const targetCode = getVal(arvore.subcategoria, ['Categoria de uso', 'categoriaUso']);
    if (targetCode) arvore.categoria = todosUsos.find(u => getVal(u, ['Código', 'Codigo', 'codigo']) === targetCode);
  }
...
export const pesquisarUsos = (
  ...
  if (modo === 'atividade') {
    const usosPorCodigo = usosData.filter(u => {
      ...
      return cod && cod.toLowerCase().includes(termoCodigo);
    });

    if (usosPorCodigo.length > 0) {
      result.tipoBusca = 'CODIGO';

      // Check if we have an EXACT match for the code (e.g. "R1" strictly matching "R1", not "nR1-1")
      const exatos = usosPorCodigo.filter(u => getVal(u, ['Código', 'Codigo', 'codigo']).toLowerCase() === termoCodigo);

      if (exatos.length > 0) {
        exatos.forEach(uso => {
          result.resultados.push({
            item: uso,
            arvore: buildUsoHierarchy(getVal(uso, ['Código', 'Codigo', 'codigo']), usosData)
          });
        });
        // If we found exact code matches (like "R1"), we just return them to avoid polluting with "nR1-1", "nR1-2", etc.
        return result;
      }
  ...
  if (modo === 'cnae' && cnaeData && cnaeData.length) {
    ...
    const cnaesEncontrados = cnaeData.filter(c => {
      const subclass = getVal(c, ['subclassesCnae2.2', 'Subclasses (CNAE 2.2)']).replace(/[.\-/]/g, '');
      const denom = normalizeString(getVal(c, ['denominacaoCnae2.2', 'Denominação (CNAE 2.2)']));
    ...
    if (cnaesEncontrados.length > 0) {
      ...
      cnaesEncontrados.forEach(cnae => {
        ...
        const codigoAlvo = getVal(cnae, ['atividade', 'Atividade', 'grupoAtividades', 'Grupo de Atividades']);
        if (codigoAlvo) {
          const usoEncontrado = usosData.find(u => getVal(u, ['Código', 'Codigo', 'codigo']) === codigoAlvo);
          if (usoEncontrado) {
            result.resultados.push({
              item: usoEncontrado,
              arvore: buildUsoHierarchy(getVal(usoEncontrado, ['Código', 'Codigo', 'codigo']), usosData),
              cnaeOriginario: cnae
            });
          }
        }
      ...
      // Remover duplicatas
      result.resultados = result.resultados.filter((v, i, a) => a.findIndex(t => (getVal(t.item, ['Código', 'Codigo', 'codigo']) === getVal(v.item, ['Código', 'Codigo', 'codigo']))) === i);
  ...
  if (modo === 'atividade') {
    ...
    const usosEncontrados = usosData.filter(u => {
      ...
      return desc && normalizeString(desc).includes(termoNormalizado);
    });
...
export const getRegrasZonamento = (
  ...
  const possibleKeys = ['categoriaUsoTipologia', 'categoriausotipologia', 'categoria de uso / tipologia'];
  const keyIdentifier = Object.keys(usosPorZonaData[0]).find(k =>
  ...
  const regra = usosPorZonaData.find(r =>
    r[keyIdentifier] && String(r[keyIdentifier]).trim().toLowerCase() === String(codigoTipologiaOuGrupo).trim().toLowerCase()
  );
...
export const getCondicoesInstalacao = (
  ...
  const possibleKeys = ['subcategoriaUsoGruposAtividadeUsosEspecificos', 'subcategoria de uso, grupos de atividade e usos específicos'];
  const keyIdentifier = Object.keys(parametrosUsoData[0]).find(k =>
    possibleKeys.includes(k.toLowerCase().trim()) ||
    k.toLowerCase().includes('tipologia') ||
    k.toLowerCase().includes('grupo')
  ) || Object.keys(parametrosUsoData[0])[0];

  const regra = parametrosUsoData.find(r =>
    r[keyIdentifier] && String(r[keyIdentifier]).trim().toLowerCase() === String(codigoTipologiaOuGrupo).trim().toLowerCase()
  );
  if (!regra) return [];
